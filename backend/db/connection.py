...
class MongoDBConnection:
    ...
    def _create_indexes(self) -> None:
        ...
        try:
            ...
            self._database.agent_memory.create_index([("performance_weight", -1)])

            # ml_models indexes (Phase 2)
            self._database.ml_models.create_index(
                [("model_type", 1), ("version", 1)]
            )
            self._database.ml_models.create_index([("trained_at", -1)])
            self._database.ml_models.create_index([("status", 1)])
...
def get_mongodb_client() -> Optional[Database]:
    """
    Get MongoDB client for direct database operations.
    Used by writers/readers that need direct DB access.
...
def setup_indexes(client: Optional[Database] = None) -> bool:
    """
    Setup indexes on all collections.
    Can be called manually to ensure indexes exist.
    
    Args:
        client: Database instance (optional, will use default if not provided)
    ...
    if client is None:
        ...
        if connection._database is None:
            logger.warning("MongoDB not available, cannot setup indexes")
            return False
        ...
        try:
            # market_data_raw indexes
            client.market_data_raw.create_index(
                [("symbol", 1), ("timestamp", -1), ("timeframe", 1)]
            )
            client.market_data_raw.create_index([("ingested_at", -1)])

            # market_data_validated indexes
            client.market_data_validated.create_index(
                [("symbol", 1), ("timestamp", -1), ("timeframe", 1)]
            )
            client.market_data_validated.create_index([("validated_at", -1)])

            # market_data_clean indexes
            client.market_data_clean.create_index(
                [("symbol", 1), ("timestamp", -1), ("timeframe", 1)]
            )

            # market_features indexes
            client.market_features.create_index(
                [("symbol", 1), ("timestamp", -1), ("timeframe", 1)]
            )
            client.market_features.create_index([("generated_at", -1)])
            client.market_features.create_index([("version", -1)])

            # normalization_params indexes
            client.normalization_params.create_index(
                [("symbol", 1), ("timeframe", 1), ("version", -1)]
            )

            # validation_log indexes
            client.validation_log.create_index(
                [("symbol", 1), ("timeframe", 1), ("validated_at", -1)]
            )

            # agent_outputs indexes
            client.agent_outputs.create_index(
                [("agent_name", 1), ("timestamp", -1)]
            )
            client.agent_outputs.create_index([("run_id", 1)])
            client.agent_outputs.create_index([("created_at", -1)])

            # agent_memory indexes
            client.agent_memory.create_index(
                [("agent_name", 1), ("timestamp", -1)]
            )
            client.agent_memory.create_index([("session_id", 1)])
            client.agent_memory.create_index([("performance_weight", -1)])

            # ml_models indexes (Phase 2)
            client.ml_models.create_index(
                [("model_type", 1), ("version", 1)]
            )
            client.ml_models.create_index([("trained_at", -1)])
            client.ml_models.create_index([("status", 1)])

            # positions_and_risk indexes (optional collection)
            client.positions_and_risk.create_index(
                [("symbol", 1), ("timestamp", -1)]
            )
